...
export async function parseVppBinary(buffer: ArrayBuffer): Promise<VppAnalysisResult> {
  ...
  // Calculate scan limit: only scan up to ~5MB past the last C# script boundary.
  // Beyond that is image data, which has no NRBF metadata.
  const boundaries = findBoundaries(text);
  const lastBoundary = boundaries.length > 0 ? boundaries[boundaries.length - 1] : 0;
  const scanLimit = Math.min(data.length, lastBoundary + 5 * 1024 * 1024);

  // Phase A: spec blocks (generic, no name filter)
  const specBlocks = extractSpecBlocks(data, scanLimit);

  // Phase B: per-script parsing
  const outputItems = parseAllScripts(text, data, warnings, boundaries, scanLimit);
...
function extractSpecBlocks(data: Uint8Array, scanLimit: number): SpecBlock[] {
  const blocks: SpecBlock[] = [];
  let i = 0;
  while ((i = data.indexOf(0x06, i)) >= 0 && i < scanLimit) {
    const sr = readLeb128(data, i + 5); if (sr.value < 2 || sr.value > 100) { i++; continue; }
    ...
    if (/SmtCO|_Smt\d/.test(str)) { i++; continue; }
    // Generic filter: clean name, no binary garbage
    if (str.length > 40 || /[\x00-\x1F\x7F-\x9F]/.test(str)) { i++; continue; }
    if (/[|\[\]]/.test(str) || str.includes('.Value') || str.includes('.Item')) { i++; continue; }
    // Skip common noise patterns
    if (/^(Output|Base|LastRun|System|Cognex|Version|Assembly)/.test(str)) { i++; continue; }
    if (/^[0-9a-f]{8}-[0-9a-f]{4}-/.test(str)) { i++; continue; }
...
// ─── NRBF input defaults extraction ───
function extractInputDefaults(data: Uint8Array, scanLimit: number): Map<string, number> {
  const defaults = new Map<string, number>();

  let i = 0;
  while ((i = data.indexOf(0x06, i)) >= 0 && i < scanLimit) {
    if (i + 60 >= scanLimit) break;
    const len = data[i + 5];
    if (len < 2 || len > 50) { i++; continue; }
    const nameStart = i + 6;
    if (nameStart + len > data.length) break;
    const name = decode(data.slice(nameStart, nameStart + len));
    if (!/^\w+$/.test(name) || name.length < 2) { i++; continue; }

    // Validate: next BinaryObjectString should be a GUID (36-char hex string)
    const id06Pos = nameStart + len;
    if (id06Pos + 6 >= data.length || data[id06Pos] !== 0x06) { i++; continue; }
    const idLen = data[id06Pos + 5];
    if (idLen !== 36) { i++; continue; }
    const idStart = id06Pos + 6;
    if (idStart + idLen > data.length) break;
    const guid = decode(data.slice(idStart, idStart + idLen));
    if (!/^[\da-f]{8}-[\da-f]{4}-[\da-f]{4}-[\da-f]{4}-[\da-f]{12}$/i.test(guid)) { i++; continue; }

    if (defaults.has(name)) { i++; continue; }
    
    // Pattern S (simple): 00 <byte> 00 00 00 — 5 bytes total
    if (i >= 5 && data[i - 5] === 0x00 && data[i - 2] === 0x00 && data[i - 1] === 0x00) {
      const val = data[i - 4];
      if (val > 0 && val <= 200) { defaults.set(name, val); i++; continue; }
    }
    // Pattern R (reference): last 4 bytes before mName 0x06 as Int32 LE
    if (i >= 4) {
      const val = new DataView(data.buffer, data.byteOffset + i - 4, 4).getInt32(0, true);
      if (val > 0 && val <= 200) { defaults.set(name, val); i++; continue; }
    }
    i++;
  }
...
function parseAllScripts(
  text: string, data: Uint8Array, warnings: string[],
  boundaries: number[], scanLimit: number,
): Map<number, OutputItemDef[]> {
  // key: script offset, value: items in that script
  const result = new Map<number, OutputItemDef[]>();
  const inputDefaults = extractInputDefaults(data, scanLimit);
  ...
  for (let i = 0; i < boundaries.length; i++) {
    ...
    for (const [rootTerm, oiVar] of bindings) {
      ...
      const pts = findArraySizeForVar(block, data4, inputDefaults);
...
function findArraySizeForVar(block: string, varName: string, inputDefaults: Map<string, number>): number | null {
  if (!varName) return null;
  // Pattern A: varName = new double[N] (numeric literal)
  const r = new RegExp(`${escapeRegex(varName)}\\s*=\\s*new\\s+double\\s*\\[\\s*(\\d+)\\s*\\]`);
  const m = r.exec(block);
  if (m) return parseInt(m[1], 10);

  // Pattern B: varName = new double[variable] (resolve the variable)
  const varR = new RegExp(`${escapeRegex(varName)}\\s*=\\s*new\\s+double\\s*\\[\\s*(\\w+)\\s*\\]`);
  const vm = varR.exec(block);
  if (vm) {
    const sizeVar = vm[1];
    // B1: sizeVar = N (numeric literal assignment, e.g. "int count = 24;")
    const numR = new RegExp(`\\b${escapeRegex(sizeVar)}\\b\\s*=?\\s*(\\d+)\\s*;`);
    const nm = numR.exec(block);
    if (nm) return parseInt(nm[1], 10);

    // B2: sizeVar = N in string literals (e.g. '"请确认count=24"')
    const hintR = new RegExp(`["'][^"']*\\b${escapeRegex(sizeVar)}\\s*=\\s*(\\d+)\\b[^"']*["']`, 'i');
    const hm = hintR.exec(block);
    if (hm) return parseInt(hm[1], 10);

    // B3: sizeVar = this.Inputs.XXX (external input, try defaults)
    const inputR = new RegExp(`\\b${escapeRegex(sizeVar)}\\s*=\\s*this\\.Inputs\\.(\\w+)\\s*;`);
    const im = inputR.exec(block);
    if (im) {
      const inputName = im[1];
      if (inputDefaults.has(inputName)) return inputDefaults.get(inputName)!;
      return null; // variable resolution failed
    }
  }

  // Pattern D: varName = new double [this.Inputs.XXX] (direct input path)
  const dirR = new RegExp(`${escapeRegex(varName)}\\s*=\\s*new\\s+double\\s*\\[\\s*this\\.Inputs\\.(\\w+)\\s*\\]`);
  const dm = dirR.exec(block);
  if (dm) {
    const inputName = dm[1];
    if (inputDefaults.has(inputName)) return inputDefaults.get(inputName)!;
    return null;
  }

  // Pattern E: fallback — new double[N] anywhere near varName in block
