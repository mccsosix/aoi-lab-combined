...
export default function SheetGeneratorTool({ onClose, showClose = true }: Props) {
                ...
                <div className="measurement-table" role="table" aria-label="测量项目">
                  ...
                  {projects.map((p, i) => (
                    <div className="measurement-row" role="row" key={i}>
                      ...
                      <label className="tolerance-input"><span className="sr-only">公差</span><input type="number" min={0} step={0.01} value={p.tolerance} onChange={e => updateProject(i, 'tolerance', Math.max(0, parseFloat(e.target.value) || 0))} /></label>
