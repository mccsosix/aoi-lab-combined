...
function blankCamera(): Camera { return { id: id("camera"), brand: "", series: "", model: "", enabled: true, hidden: false, resolutionWidthPx: 0, resolutionHeightPx: 0, sensorWidthMm: 0, sensorHeightMm: 0, sensorDiagonalMm: 0, lensMount: null, megapixels: 0, nominalMegapixels: null, sensorFormat: null, fps: null, pixelSizeXUm: null, pixelSizeYUm: null, sensorModel: null, productUrl: null, source: "user", customized: true, updatedAt: now() }; }
function blankLens(): Lens { return { id: id("lens"), brand: "", series: "", model: "", lensType: "物方远心", enabled: true, hidden: false, magnification: 0, magnificationMin: null, magnificationMax: null, nominalMagnification: null, mount: null, maxSensorDiagonalMm: null, workingDistanceMm: null, depthOfFieldMm: null, depthOfFieldMinMm: null, depthOfFieldMaxMm: null, depthOfFieldSymmetric: false, depthOfFieldAperture: null, aperture: null, sensorFormat: null, imageMtfLpMmMin: null, objectSpaceResolutionUm: null, resolutionSource: null, dataQuality: null, conflicts: null, aliases: [], recommendationStatus: "manual-review", sourceFiles: [], notes: [], source: "user", customized: true, updatedAt: now() }; }
...
function LensFields({ item, onChange }: { item: Lens; onChange: (item: Lens) => void }) {
  return <>
    ...
    <label>直接物方解析度（μm）<input type="number" min="0" step="any" value={item.objectSpaceResolutionUm ?? ""} onChange={(e) => onChange({ ...item, objectSpaceResolutionUm: nullableNumber(e.target.value) })} placeholder="厂家规格书给出时填写" /></label>
    <label>解析度来源<select value={item.resolutionSource ?? ""} onChange={(e) => onChange({ ...item, resolutionSource: (e.target.value || null) as Lens["resolutionSource"] })}>
      <option value="">（自动判断）</option>
      <option value="manufacturer-object-space">厂家物方解析度</option>
      <option value="mtf-estimate">基于像方 MTF 估算</option>
    </select></label>
    ...
    <label>DOF 下限（mm）<input type="number" min="0" step="any" value={item.depthOfFieldMinMm ?? ""} onChange={(e) => onChange({ ...item, depthOfFieldMinMm: nullableNumber(e.target.value) })} /></label>
    <label>DOF 上限（mm）<input type="number" min="0" step="any" value={item.depthOfFieldMaxMm ?? ""} onChange={(e) => onChange({ ...item, depthOfFieldMaxMm: nullableNumber(e.target.value) })} /></label>
