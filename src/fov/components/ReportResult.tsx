...
export function ReportResult({ camera, lens, fov, compatibility }: { camera: Camera; lens: Lens; fov: FovResult; compatibility: CompatibilityResult }) {
  ...
  const sensor = `${report.nominalMegapixels === null ? "—" : `${compact(report.nominalMegapixels)}M`}${report.sensorFormat ? `（${report.sensorFormat}）` : ""}/ fps=${compact(report.fps)}`;
  ...
  return <>
    ...
    <dl className="report-list">
      ...
      <ReportRow
        label="DOF"
        value={report.dofMm === null ? "—" : `${dofSign}${fixed(report.dofMm, 2)} mm`}
        detail={report.dofMm === null ? undefined : `${dofSign}${fixed(report.dofSourceMm, 2)} mm × ${magnification(report.magnification)}`}
      />
      <ReportRow label="Resolution" value={`${fixed(report.resolutionMmPerPixel, 5)} mm/pixel`} />
      <ReportRow label="Lens Resolution" value={report.lensResolutionMm === null ? "—" : `${fixed(report.lensResolutionMm, 3)} mm/pixel`} />
    </dl>
    ...
    <details className="result-details">
      ...
      <p className="calculation-note">DOF 按项目约定使用“镜头资料值 × 实际倍率”；Lens Resolution 为基于镜头图像侧 MTF 的半周期估算值。</p>
    </details>
  </>;
}
