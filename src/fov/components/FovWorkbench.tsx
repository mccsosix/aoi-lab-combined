...
export function FovWorkbench({ onBack, repository = indexedDbDeviceRepository, defaultData = bundledDefaults }: { onBack?: () => void; repository?: DeviceRepository; defaultData?: DefaultDeviceData }) {
  ...
  const manual = useMemo<{ fov: FovResult; compatibility: CompatibilityResult } | { error: string } | null>(() => {
  ...
  return <main className="fov-shell">
    <div className="fov-page">
      <header className="fov-header">
        <button type="button" className="back-link" onClick={() => { if (onBack) onBack(); else window.history.back(); }}>← 返回工具箱</button>
