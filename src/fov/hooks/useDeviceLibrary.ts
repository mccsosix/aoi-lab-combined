...
const DEFAULT_DATA_VERSION = "2026-08-12-camera1147-lens426";
...
export function useDeviceLibrary(repository: DeviceRepository, defaults: DefaultDeviceData) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await repository.initializeDefaults(defaults, DEFAULT_DATA_VERSION);
  ...
  const reset = async () => { await repository.resetDefaults(); await repository.initializeDefaults(defaults, DEFAULT_DATA_VERSION); await refresh(); };
