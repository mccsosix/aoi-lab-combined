...
export function useDeviceLibrary(repository: DeviceRepository, defaults: DefaultDeviceData) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await repository.initializeDefaults(defaults, "2026-08-12-camera1135-lens623");
  ...
  const reset = async () => { await repository.resetDefaults(); await repository.initializeDefaults(defaults, "2026-08-12-camera1135-lens623"); await refresh(); };
